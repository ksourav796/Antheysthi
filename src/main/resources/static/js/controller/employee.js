...
app.controller('employeeCtrl',
    function ($scope, $timeout,$http, $location, $filter, Notification,$rootScope) {
        ...
        $scope.getEmployeeListBasedOnParent = function () {
            $http.post($scope.hiposServerURL+"/getEmployeeListBasedOnParent").then(function (response) {
                var data = response.data;
                $scope.employeeList = data;
            })
        };
        $scope.getEmployeeListBasedOnParent();
                        ...
                        $http.post($scope.hiposServerURL +  '/deleteEmployee', angular.toJson(deleteDetails, "Create")).then(function (response, status, headers, config) {
                            ...
                            if($scope.status=="InActive") {
                                ...
                                if($scope.empStatusText=="InActive") {
                                    $scope.getEmployeeListBasedOnParent("","InActive");
                                    Notification.success({

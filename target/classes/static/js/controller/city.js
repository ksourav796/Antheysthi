app.controller('cityController',
    function ($scope, $timeout, $http, $location, $filter, Notification,$window) {
        ...
        $scope.getpaginatedcitylist = function (page) {
            switch (page) {
                case 'firstPage':
                    $scope.firstPage = true;
                    $scope.lastPage = false;
            ...
            $http.post($scope.hiposServerURL + "/getpaginatedcity?&type=" + $scope.inactiveStatus+ '&searchText=' + $scope.searchText, angular.toJson(paginationDetails)).then(function (response) {
                var data = response.data;
                console.log(data);
        ...
        $scope.saveCity = function () {
            if (angular.isUndefined($scope.countryId) || $scope.countryId == '' || $scope.countryId == null) {
                ...
                $http.post($scope.hiposServerURL + '/saveCity', angular.toJson(saveCityDetails, "Create")).then(function (response, status, headers, config) {
                    var data = response.data;
                    if (data == "") {
